...
//			}
		...
		$render( forWhat ){
			...
			let $gridBody = $('<div style="border:1px solid #d5dbe3;padding:0;box-shadow: 2px 2px #d5dbe3;overflow-x:auto;width:fit-content;max-width:100%;">').appendTo($grid);
	...
	class AdvancedSearch{
		...
		constructor( structuredDataList, $querySection, $resultSection, baseLinkURL ){
			...
			Liferay.on(Events.SEARCH_KEYWORD_CHANGED, function(event){
				...
				let gridTerm;
				if( dataPacket.term.isColumn() ){
					gridTerm = term.gridTerm;
				}

				let searchHistory;
				if( advancedSearch.searchHistories.length < 1 ){
					advancedSearch.searchHistories.push(new SearchHistory(structuredDataList));
				}
				...
				if( (!dataPacket.rangeSearch &&
					...
					//advancedSearch.searchHistories.push( searchHistory );

					return;
				}

				let query;
				let fieldName = term.isColumn() ? gridTerm.termName : term.termName;
				let fieldType = term.isColumn() ? TermTypes.GRID : term.termType;
				let columnName = term.isColumn() ? term.termName : undefined;
				let columnType = term.isColumn() ? term.termType : undefined;
				let operator;
				let keywords;

				let searchType = term.isColumn() ? columnType : fieldType;
				switch( searchType ){
					case TermTypes.STRING:
					case TermTypes.PHONE:
					case TermTypes.EMAIL:
				...
				if( keywords.length > 1 ){
					query = new Query( advancedSearch.infieldOperator, null );
					keywords.forEach( keyword => {
						term.isColumn() ? 
								searchHistory.addGridKeyword(
									fieldName,
									columnName, 
									columnType, 
									keyword, 
