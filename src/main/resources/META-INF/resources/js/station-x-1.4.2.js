let StationX = function ( NAMESPACE, DEFAULT_LANGUAGE, CURRENT_LANGUAGE, AVAILABLE_LANGUAGES ) {
	...
	let FormUIUtil = {
		...
		$getCheckboxTag: function( controlId, controlName, label, checked, value, disabled, eventFuncs ){
			...
			let $input = $( '<input type="checkbox" style="display:inline-block;margin-right:10px;">').appendTo( $label );
			$input.prop({
				class: "field",
				id: controlId,
				name: controlName,
		...
		$getFieldsetTag: function( controlName, label, mandatory, helpMessage ){
			...
			let $fieldset = $('<fieldset style="width:fit-content;max-width:100%;min-width:fit-content;border: 1px solid #aaa; box-shadow: 2px 2px #ccc;">').prop({
				'id': controlName,
				'name': controlName
			});
		...
		$getFieldSetGroupNode : function( controlName, label, mandatory, helpMessage ){
			let $label = this.$getLabelNode( label, mandatory, helpMessage );
	...
	class Term {
		...
		/**
		 * v.0.1.0
		 * Gets search index of a term or a column
		 * 
		 * @param {JSONObject} searchData 
		 * @param {Term} term 
		 * @param {string} columnName 
		 * @returns
		 * 	- Array of data if term type is grid,
		 *  - just data otherwise.
		 */
		static getSearchIndex( searchData, term, columnName ){
			if( term.isColumn() ){
				switch( term.termType ){
					...
					case TermTypes.PHONE:{
						let gridData = GridTerm.getColumnData( searchData[term.termName], columnName );
						break;
					}
					case TermTypes.FILE:{
	...
	class NumericTerm extends Term{
		...
		$getEditorNumericNode(){
			...
			if( Util.isNotEmpty( this.minValue) ){
				$('<span style="display:inline-block;min-width:fit-content;max-width:fit-content;text-align:center;width:fit-content;"><strong>' +
				...
				$('<span style="display:inline-block;min-width:fit-content;max-width:fit-content;width:fit-content;text-align:center;margin:3px 3px;"><strong>' +
			...
			if( this.uncertainty ){
				$('<div style="display:inline-block;;min-width:fit-content;max-width:fit-content;width:fit-content;text-align:center;margin:0 5px 0 5px;"><strong>&#xB1;</strong></div>')
					.appendTo( $node );
		...
		set value(value){ 
			if( Util.isNotEmptyString(value) ){
				if( value.startsWith('[') && value.endsWith(']') ){
					...
					if( this.displayStyle === ListTerm.DISPLAY_STYLE_CHECK ){
						this.#value = values;
					}
					else{
		...
		$getControlNode( forWhat ){
			...
			if( forWhat === Constants.FOR_SEARCH ){
				...
				$node.change(function(event){
					...
					let dataPacket = Util.createEventDataPacket(NAMESPACE,NAMESPACE);
					dataPacket.term = term;
					dataPacket.keywords = FormUIUtil.getFormCheckedArray( term.termName );
				...
				if( this.displayStyle === ListTerm.DISPLAY_STYLE_SELECT ){
					let optionValue = this.hasValue() ? this.#value[0] : '';
	...
	//							'<path class="lexicon-icon-outline" d="M256 0c-141.37 0-256 114.6-256 256 0 141.37 114.629 256 256 256s256-114.63 256-256c0-141.4-114.63-256-256-256zM269.605 360.769c-4.974 4.827-10.913 7.226-17.876 7.226s-12.873-2.428-17.73-7.226c-4.857-4.827-7.285-10.708-7.285-17.613 0-6.933 2.428-12.844 7.285-17.788 4.857-4.915 10.767-7.402 17.73-7.402s12.932 2.457 17.876 7.402c4.945 4.945 7.431 10.854 7.431 17.788 0 6.905-2.457 12.786-7.431 17.613zM321.038 232.506c-5.705 8.923-13.283 16.735-22.791 23.464l-12.99 9.128c-5.5 3.979-9.714 8.455-12.668 13.37-2.955 4.945-4.447 10.649-4.447 17.145v1.901h-34.202c-0.439-2.106-0.731-4.184-0.936-6.291s-0.321-4.301-0.321-6.612c0-8.397 1.901-16.413 5.705-24.079s10.24-14.834 19.309-21.563l15.185-11.322c9.070-6.7 13.605-15.009 13.605-24.869 0-3.57-0.644-7.080-1.901-10.533s-3.219-6.495-5.851-9.128c-2.633-2.633-5.969-4.71-9.977-6.291s-8.66-2.369-13.927-2.369c-5.705 0-10.561 1.054-14.571 3.16s-7.343 4.769-9.977 8.017c-2.633 3.247-4.594 7.022-5.851 11.322s-1.901 8.66-1.901 13.049c0 4.213 0.41 7.548 1.258 10.065l-39.877-1.58c-0.644-2.311-1.054-4.652-1.258-7.080-0.205-2.399-0.321-4.769-0.321-7.080 0-8.397 1.58-16.619 4.74-24.693s7.812-15.214 13.927-21.416c6.114-6.173 13.663-11.176 22.645-14.951s19.368-5.676 31.188-5.676c12.229 0 22.996 1.785 32.3 5.355 9.274 3.57 17.087 8.25 23.435 14.014 6.319 5.764 11.089 12.434 14.248 19.982s4.74 15.331 4.74 23.289c0.058 12.581-2.809 23.347-8.514 32.27z"></path>' +
		...
		$getControlNode( forWhat ){
			...
			else if( forWhat === Constants.FOR_PREVIEW ){
	...
	class GridTerm extends Term{
		...
		$getColumnCells( colName ){
			...
			let keys = Object.keys( this.#$renderedCells );
			keys.forEach( key => columnCells.push( this.#$renderedCells[key][colName] ) );
		...
		#removeColumnRenders( colName ){
			...
			let renders = this.#getColumnRenders( colName );
...
//				$parent = $cell.parent();
		...
		#$createNumericCell( rowIndex, colDef, forWhat ){
			...
			if( forWhat === Constants.FOR_EDITOR || forWhat === Constants.FOR_PREVIEW ){
				...
				if( Util.isNotEmpty( colDef.minValue) ){
					$('<span style="display:inline-block;min-width:fit-content;max-width:fit-content;text-align:center;width:fit-content;">' +
					...
					$('<span style="display:inline-block;min-width:fit-content;max-width:fit-content;width:fit-content;text-align:center;margin:3px 3px;">' +
		...
		#$createListCell( rowIndex, colDef ){
			...
			let $select = FormUIUtil.$getSelectTag(
				controlName,
				options,
				value,
				placeHolder,
		...
		setColumnSelected( colName, on=true, forWhat ){
			...
			let gridTerm = this;
			if( on ){
				let $columnCells = this.#getColumnRenders( colName );
		...
		$getControlNode( forWhat ){
			let $gridBody = $('<div style="border:1px solid #d5dbe3;padding:0;box-shadow: 2px 2px #d5dbe3;overflow-x:auto;width:fit-content;max-width:100%;">');
			let $table = $('<table>').appendTo( $gridBody );
...
//		get $displayStyle(){ return $('#'+NAMESPACE+'displayStyle');}
	...
	class FieldKeyword extends Query{
		...
		$renderHit( hit, order, $canvas ){
			console.log( '$renderHit: ', hit );

			let $row = $('<div style="display:flex;padding-top:3px; padding-bottom:3px;width:100%;">').appendTo($canvas);
			
			let $orderCol = $('<div style:"text-align:right;">').text( order );
			$row.append( $orderCol );
			
			let $dataCol = $('<div>').appendTo( $row );

			let data = this.field + ':' + hit.data[this.fieldName];
			$dataCol.text( data );
			/*
			let $href = $('<a>');
	...
	class GridKeyword extends FieldKeyword{
	...
	class SearchHistory{
		...
		constructor( dataList ){
			this.rootQuery = new Query( Constants.SearchOperators.AND );
			this.queries = [this.rootQuery];
			this.dataList = dataList;
		}

		/**
		 * adds a query to the query stack.
		 * 
		 * @param {Query} query 
		 */
		addQuery( query ){
		...
		addFieldKeyword(fieldName, fieldType, keyword, operator, parentId ){ 
			this.queries.push( new FieldKeyword( fieldName, fieldType, keyword, operator, parentId ) );
		}

		addGridKeyword(fieldName, columnName, columnType, keyword, operator, parentId ){ 
			this.queries.push( new GridKeyword( fieldName, columnName, columnType, keyword, operator, parentId ) );
		}
		...
		isGridQuery( query ){
			return query instanceof GridKeyword;
		}
		...
		#doRangeSearch( query, dataList ){
			...
			query.hits = dataList.filter( data => {
				let fieldIndex = data[query.fieldName];
				if( !fieldIndex ) return Constants.FILTER_SKIP;


				let searchIndex = this.isGridQuery(query) ?
										GridTerm.getColumnData( fieldIndex, query.columnName ):
										fieldIndex;

				if( this.isGridQuery( query ) ){
					let conform = false;
					searchIndex.every( index => {
						let value = (typeof index === 'object') ? index.value : index;
					...
					let value = (typeof searchIndex === 'object') ? searchIndex.value : searchIndex
		...
		#doLikeSearch( query, dataList ){
			...
			query.hits = dataList.filter( data => {
				console.log( 'doLikeSearch: ', query, data );
				let fieldContent = data.data[query.fieldName];
				if( Util.isEmpty(fieldContent) ) return Constants.FILTER_SKIP;

				let searchContent = this.isGridQuery(query) ?
										GridTerm.getColumnData( fieldContent, query.columnName ):
										fieldContent;
				//if( search)
		...
		getQueryById( queryId ){
			return this.query.getQueryById( queryId );
		}
		...
		showSearchResults( queryId, $canvas ){
			...
			query.renderHits( 0, 20, $canvas );
		}

		updateQueryOperator( queryId, operator ){
			this.query.updateQueryOperator( queryId, operator );
		}
	...
	class AdvancedSearch{
		...
		get currentHistory(){ 
			return (this.#searchHistories.length > 0) ? 
							this.#searchHistories[this.#searchHistories.length-1] :
							null;
		}
		...
		constructor( structuredDataList, $querySection, $resultSection, baseLinkURL ){
			...
			Liferay.on(Events.SEARCH_KEYWORD_CHANGED, function(event){
				...
				let searchType, isColumn;

				if( isColumn = term.isColumn() ){
				...
				let baseQuery = advancedSearch.writingQueryRoot;
				if( !!gridTerm ){
					advancedSearch.writingQuery.every( queryUnit => {
						if( queryUnit instanceof GridKeyword ){
							parentQueryId = queryUnit.parentId;
							return Constants.STOP_EVERY;
						}
					...
					baseQuery = new Query( advancedSearch.infieldOperator, parentQueryId );
				}

				if( keywords.length > 1 ){
					let query = new Query( advancedSearch.infieldOperator, advancedSearch.writingQueryRoot );

					keywords.forEach( keyword => {
						term.isColumn() ? 
								new GridKeyword( fieldName, columnName, columnType, keyword, operator, query.id ) :
				...
				if( Util.isEmpty(advancedSearch.writingQuery) ){
					advancedSearch.writingQuery = query;
				}
				else{
					let query;
				...
				advancedSearch.openQueryEditor( searchHistory, advancedSearch.$querySection );
			});
		...
		*/
		...
		getWritingQueryByFiledName( fieldName ){
			let wQuery;
			this.#writingQuery.every( query => {
		...
		getParantWritingQuery( fieldName ){
			...
			this.#writingQuery.every( query => {
				if( query.fieldName === fieldName ){
					parentQuery = this.getWritingQueryById( query.parentId );
					return Constants.STOP_EVERY;
				}
		...
		removeWritingQuery( fieldName ){

		}

		addWritingQueryToHistory(){
			if( Util.isEmpty(this.#writingQuery) )	return;
			...
			currentHistory.addQuery( this.writingQueryRoot, this.writingQuery, this.fieldOperator );

			this.writingQueryRoot = null;
			this.writingQuery = new Array();
		}

		openQueryEditor( searchHistory, $canvas ){
			let advancedSearch = this;
		...
		fetchFieldKeyword( fieldName ){
			return this.searchHistories.find( history => history.fieldName === fieldName);
		}
